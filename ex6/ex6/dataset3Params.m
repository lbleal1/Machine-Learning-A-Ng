...
prediction_error = zeros(length(C_list), length(sigma_list)); %8x8
result = zeros(length(C_list)+length(sigma_list),3); % 16 x 3
row = 1;
...
for i = 1:length(C_list),
  for j = 1: length(sigma_list),
    ...
    result(row,:) = [prediction_error(i,j), C_list(i), sigma_list(j)];
    row = row + 1;
   end
end
